...
public:
    ...
    *      map.insert(val);
    */
    using value_type = std::pair<const K, M>;  // key should not be modified
    ...
    */
    ...
    //this is copy assignment function
    HashMap<K, M, H>& operator=(const HashMap& map);

    // this is move constructor
    HashMap(HashMap&& map) noexcept;

    // this is move assignment
    HashMap<K, M, H>& operator=(HashMap&& map) noexcept;
